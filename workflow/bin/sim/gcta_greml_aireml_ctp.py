...
def main():
    ...
    for key in data.keys():
        ...
        for i in range(C):
            ...
            # run GCTA
            try:
                util.subprocess_popen( cmd1, log_fn=prefix + '.ai.log' )
                alg = 'AI'
            except:
        ...
        try:
            util.subprocess_popen( cmd1 + ['--qcovar', qcovar_f], log_fn=prefix + '.ai.log2' )
            alg2 = 'AI'
        except:
        ...
        # save
        outs.append({'gene': key, 'greml_ctp': greml, 'greml_op': greml_op, 'alg': alg, 'log': open(prefix + '.ai.log').read(), 
                     'alg2': alg2, 'log2': open(prefix + '.ai.log2').read()})

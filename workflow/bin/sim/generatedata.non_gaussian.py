...
def main():
    ...
    dist_g = snakemake.wildcards.dist_g
    dist_v = snakemake.wildcards.dist_v
    # q = 0.5 # spike-slab parameter pi: probability of spike
    is_slab = np.array([1] * int(L / 2) + [0] * (L - int(L / 2)))
    slab_sd_scale = 1.0 # spike-slab parameter slab_sd = scale * sqrt(sig_g / L)
    spike_sd_scale = 0.1 # spike-slab parameter spike_sd = scale * sqrt(sig_g / L)

    # simulate data
    data = {}
    for i in batch:
        rng = np.random.default_rng(snakemake.params.seed + i)
        # is_slab = rng.binomial(1, 1 - q, snakemake.params.L)
        data[i] = {}
        ...
        if sig_g == 0:
            ...
            if dist_g == 'gaussian':
                add = rng.normal(0, 1, L)
            elif dist_g == 't2':
                add = rng.standard_t(2, L)
            elif dist_g == 'spikeslab':
                add = rng.normal(0, slab_sd_scale / np.sqrt(is_slab.sum()), L) * is_slab
                add = add + rng.normal(0, spike_sd_scale / np.sqrt((1 - is_slab).sum()), L) * (1 - is_slab)
            add = add - np.mean(add)
        ...
        if np.all(V == np.zeros_like(V)):
            ...
            if dist_v == 'gaussian':
                ...
                H = stats.multivariate_t.rvs(np.zeros(C), V / L, 2, L, random_state=rng)
            elif dist_v == 'spikeslab':
                H = np.array([rng.multivariate_normal(np.zeros(C), V * slab_sd_scale**2 / is_slab.sum()) if is_slab[j] == 1 else 
                              rng.multivariate_normal(np.zeros(C), V * spike_sd_scale**2 / (1 - is_slab).sum()) for j in range(L)])

...
def main():
    ...
    cts = [ct.replace(' ', '') for ct in P.columns.tolist()]
    out = np.load(snakemake.input.out, allow_pickle=True).item()
    df = util.read_out(out, ['gene', 'p:free:V', 'p:free:hom_g2', 'p:free:vc'], cts)
    ...
    elif snakemake.wildcards.gset in cts:
        p_cut = 0.05 / len(out['gene'])
        target_genes = df.loc[(df['p:free:V'] < p_cut) & (df[f'p:free:vc_{snakemake.wildcards.gset}'] < p_cut), 'gene'].values
    else:
    ...
    if randoms.shape[0] < k:
        print(randoms.shape)
        sys.exit(f'Warning: only {randoms.shape[0]} random sets generated, need {len(snakemake.params.random)}')

    with tarfile.open(snakemake.output.random, 'w') as tar:
        for i, f in enumerate(snakemake.params.random):

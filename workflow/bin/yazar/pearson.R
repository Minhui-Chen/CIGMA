...
# load sce
sce <- readRDS(snakemake$input[['sce']])  # TODO: update to use sce as input on 2023.12.20
...
# Serat to AnnData  # TODO: use zellkonverter
adata_seurat <- sc$AnnData(
    X   = t(GetAssayData(seurat)),
    obs = seurat[[]],
    var = GetAssay(seurat)[[]]  # TODO: empty?
)

adata_seurat <- py_to_r(adata_seurat)

adata_seurat$write_h5ad(snakemake$output[["sct"]])

...
def plot_hom(data, true_hom_g2, true_hom_e2, ax, boxcolor, pointcolor):
    ...
    xs = plot_help.snsbox_get_x(len(np.unique(data['arg'])), 1)
...
def plot_free(data, C, true_hom_g2s, true_hom_e2s, true_V, true_W, true_h2, 
        boxcolor, pointcolor, colorpalette, fig_f):
    '''
    ...
    # hom_g2
    sns.boxplot(x='arg', y='hom_g2', data=data, ax=axes[0,0], color=boxcolor)
    ...
    xs = plot_help.snsbox_get_x(len(np.unique(h2_df['arg'])), len(np.unique(h2_df['variable'])))
...
def main(): 
    ...
    for arg, out_f in zip(np.array(subspace[snakemake.wildcards.arg]), snakemake.input.out):
        ...
        if 'free' in snakemake.wildcards.model:
            if method:

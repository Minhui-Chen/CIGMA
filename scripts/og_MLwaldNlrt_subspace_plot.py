...
def main():
    ...
    for f in data['out']:
        ...
        # wald
        wald = out['ml']['wald'] # structure: wald - model (e.g. hom_p) - statistics (e.g. hom2, beta)
        ...
        # lrt
        lrt = out['ml']['lrt'] # structure: lrt - two comparing model (e.g. full_free)
    ...
    axes[2].plot(data['arg'], wald_power['free']['W'], marker=markers[0], label='$\sigma_{noise}^2$')
    ...
    axes[2].set_xlabel(snakemake.wildcards.arg)
    axes[2].legend()
    ...
    for x in lrt_power.keys():
        if re.search('^hom', x):
            axes[3].plot(data['arg'], lrt_power[x], marker=markers[a], label=format_name(x), 
                    color=snakemake.params.mycolors[0])
            a += 1
        if re.search('^iid', x):
            axes[3].plot(data['arg'], lrt_power[x], marker=markers[b], label=format_name(x), 
                    color=snakemake.params.mycolors[1])
            b += 1
        if re.search('^free', x):
            axes[3].plot(data['arg'], lrt_power[x], marker=markers[c], label=format_name(x), 
                    color=snakemake.params.mycolors[2])
            c += 1
        if re.search('^full', x):
            axes[3].plot(data['arg'], lrt_power[x], marker=markers[d], label=format_name(x), 
                    color=snakemake.params.mycolors[3])
            d += 1
    axes[3].legend()
    axes[3].set_title('LRT')
    axes[3].set_xlabel(snakemake.wildcards.arg)
    ...
    fig.tight_layout()
    fig.savefig(snakemake.output.waldNlrt)

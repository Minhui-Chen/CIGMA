...
def main():
    ...
    # Free model
    ## gen
    gen['hom'] = ctp['free']['hom_g2']
    V = np.diagonal(ctp['free']['V'], axis1=1, axis2=2)
    ...
    ## trans
    if 'Vt' in ctp['free'].keys():
        trans = pd.DataFrame()

        trans['hom'] = ctp['free']['hom_gt2']
        Vt = np.diagonal(ctp['free']['Vt'], axis1=1, axis2=2)
    ...
    for xtick, mean in zip(axes[0].get_xticks(), gen_means):
        ...
        axes[0].text(x, y_loc, f"{mean:.4f}", ha='center', va='center', transform=axes[0].transAxes)
    ...
    for xtick, mean in zip(axes[1].get_xticks(), env_means):
        ...
        axes[1].text(x, y_loc, f"{mean:.2f}", ha='center', va='center', transform=axes[1].transAxes)

    axes[0].axhline(0, ls='--', color='0.8', zorder=0)
    axes[1].axhline(0, ls='--', color='0.8', zorder=0)
    axes[1].set_xlabel('')
    axes[0].set_ylabel('Genetic variance (V)', fontsize=16)
    ...
    if 'Vt' in ctp['free'].keys():
        ...
        axes[2].set_xlabel('')
        axes[2].set_ylabel('Trans variance', fontsize=16)
    ...
    # p value plot
    if 'p' not in ctp.keys():
        with open(snakemake.output.p_png, 'w') as f:
            pass
    else:
        fig, axes = plt.subplots(ncols=3, figsize=(18, 4))

        draw.scatter(-1 * np.log10(ctp['p']['free']['ct_beta']), -1 * np.log10(ctp['p']['free']['V']), 
        ...
        draw.scatter(-1 * np.log10(ctp['p']['free']['V']), -1 * np.log10(ctp['p']['free']['W']), 
